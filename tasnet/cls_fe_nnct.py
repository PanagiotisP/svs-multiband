...
class Synthesis(nn.Module):
    ...
    def __init__(self, ft_size=1024, kernel_size=1024, hop_size=384, exp_settings={}):
        ...
        self.fully_modulated = exp_settings['fully_modulated']
        self.monaural = exp_settings['monaural']
        self.batch_size = None
        self.time_domain_samples = None
        self.sz_in = ft_size
        ...
        # Model parameters to be optimized
        if self.fully_modulated:
            self.norm_factor = nn.Parameter((torch.ones(self.sz_in, 1 if self.monaural else 2, self.kernel_sz) *
                                             1./(self.sz_in + self.kernel_sz)).cuda().float())
        else:
            self.norm_factor = nn.Parameter((torch.ones(1, 1 if self.monaural else 2, self.kernel_sz)
                                             * 1. / (self.sz_in + self.kernel_sz)).cuda().float())

        mel = (torch.linspace(to_mel(30), to_mel(22050), self.sz_in)).view(self.sz_in, 1).repeat(1,1 if self.monaural else 2).view(self.sz_in, 1 if self.monaural else 2, 1)
        self.freq_mat = nn.Parameter(to_normalized_hz(mel, exp_settings['fs']).cuda().float())
        self.phi = nn.Parameter(torch.randn(self.sz_in, 1 if self.monaural else 2, 1).cuda().float())
    ...
    def initialize(self):
        ...
        self.f_matrix = torch.autograd.Variable(torch.from_numpy(self.f_matrix[:, None, :]).repeat(1, 1 if self.monaural else 2, 1),
                                                requires_grad=True).cuda().float()
    ...
    def forward(self, x_coeff, use_sorting=False):
        ...
        if use_sorting:
            f_sort_mat, sorted_indices = self.freq_mat.pow(self.p).sort(dim=0)
            if self.fully_modulated:
#                 print('norm_factor before', self.norm_factor.shape)
                norm_factor = self.norm_factor.gather(0, sorted_indices.repeat(1,1,self.norm_factor.shape[-1]))
#                 print('norm_factor after', norm_factor.shape)
            else:
                norm_factor = self.norm_factor

            cos_dict = torch.cos(self.f_matrix * f_sort_mat + self.phi.gather(0, sorted_indices.repeat(1,1,self.phi.shape[-1]))) * norm_factor
            if self.monaural:
                x_coeff = x_coeff[:, sorted_indices[:, 0, 0], :]
            else:
                x_coeff = torch.stack([x_coeff,x_coeff], dim=2)
                x_coeff[:, :, 0] = x_coeff[:, :, 0][:, sorted_indices[:, 0, 0], :]
                x_coeff[:, :, 1] = x_coeff[:, :, 1][:, sorted_indices[:, 1, 0], :]
#             print('x_coeff after gather', x_coeff.shape)
    
        else:
            cos_dict = torch.cos(self.f_matrix * self.freq_mat.pow(self.p) + self.phi) * self.norm_factor

        # Reshaping
#         print('fmatrix', self.f_matrix.shape, 'freq_mat', self.freq_mat.shape, 'phi', self.phi.shape, 'norm_factor', norm_factor.shape, 'cos_dict', cos_dict.shape)
...
#             print('x_coeff', x_coeff[:, :, 0].shape)
#             print('cosdict', cos_dict[:, 0].view(self.sz_in, 1, -1).shape)
            wave_form_l = F.conv_transpose1d(x_coeff[:, :, 0], cos_dict[:, 0].view(self.sz_in, 1, -1), bias=None, stride=self.hop,
                                       padding=self.pad,
                                       dilation=1, groups=1)
            wave_form_r = F.conv_transpose1d(x_coeff[:, :, 1], cos_dict[:, 1].view(self.sz_in, 1, -1), bias=None, stride=self.hop,
                                       padding=self.pad,
                                       dilation=1, groups=1)
            wave_form = torch.cat([wave_form_l, wave_form_r], dim=1)
#         print('wave_form', wave_form.shape)
    ...
    def get_cos_dictionary(self, use_sorting=True):
        if use_sorting:
            f_sort_mat, sorted_indices = self.freq_mat.pow(self.p).sort(dim=0)
            if self.fully_modulated:
                norm_factor = self.norm_factor.gather(0, repeat(1,1,self.norm_factor.shape[-1]))
            else:
                norm_factor = self.norm_factor

            cos_dict = torch.cos(self.f_matrix * f_sort_mat + self.phi.gather(0, repeat(1,1,self.phi.shape[-1]))) * norm_factor
