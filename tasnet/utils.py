...
def sdr_objective(estimation, origin, mask=None):
    ...
    """
    if len(estimation.shape) == 3 or len(origin.shape) == 3:
        estimation = estimation.unsqueeze(0)
        origin = origin.unsqueeze(0)
#     estimation = estimation[:, 1].unsqueeze(1)
#     origin = origin[:, 1].unsqueeze(1)

    origin_power = th.pow(origin, 2).sum(dim=-1, keepdim=True) + 1e-5  # shape: (B, 4, 2, 1)
    scale = th.sum(origin*estimation, dim=-1, keepdim=True) / origin_power  # shape: (B, 4, 2, 1)
    est_true = scale * origin  # shape: (B, 4, 2, T)
    est_res = estimation - est_true  # shape: (B, 4, 2, T)
    
    true_power = th.pow(est_true, 2).sum(dim=-1).clamp(min=1e-5)  # shape: (B, 4, 2)
    res_power = th.pow(est_res, 2).sum(dim=-1).clamp(min=1e-5)  # shape: (B, 4, 2)
        
    sdr = 10*(th.log10(true_power/(res_power + 1e-5) + 1e-5))  # shape: (B, 4, 2)
    if mask is not None:
            sdr = (sdr*mask).sum(dim=(0, -1)) / mask.sum(dim=(0, -1)).clamp(min=1e-5)  # shape: (4)
    else:
