...
def generate_mpgtf(samplerate_hz, length_in_seconds, N, n_center_freqs=33, phase_split=False):
    if phase_split:
        positive_phases_idx = []
        negative_phases_idx = []
    ...
    erb_delta = freq_hz_2_erb_scale(samplerate_hz/2) - freq_hz_2_erb_scale(center_freq_hz_min)
#     print(np.ceil(freq_hz_2_erb_scale(samplerate_hz/2)), freq_hz_2_erb_scale(100))
    freqs = np.linspace(freq_hz_2_erb_scale(center_freq_hz_min), np.ceil(freq_hz_2_erb_scale(samplerate_hz/2)), n_center_freqs)
    ...
    for i in range(n_center_freqs):
        ...
        for phase_index in range(phase_pair_count[i]):
            ...
            if phase_split:
                positive_phases_idx.append(index)
        ...
        if phase_split:
            negative_phases_idx = negative_phases_idx + [x for x in range(index, index+phase_pair_count[i])]
...
#         current_center_freq_hz = erb_scale_2_freq_hz(freq_hz_2_erb_scale(current_center_freq_hz)+1)
        current_center_freq_hz = erb_scale_2_freq_hz(freqs[i])

    filterbank = normalize_filters(filterbank)
    
    if phase_split:
        return filterbank[positive_phases_idx + negative_phases_idx]
    return filterbank
